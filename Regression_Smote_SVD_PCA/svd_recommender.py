...
class SVDRecommender(object):
    ...
    def recommender_svd(self, R: np.ndarray, k: int) ->Tuple[np.ndarray, np
		...
		"""
        U, S, V_T = self.imgcompression.svd(R)
        S_k = np.diag(np.sqrt(S[:k]))

        return U[:, :k] @ S_k, S_k @ V_T[:k, :]
    ...
    def predict(self, R: np.ndarray, U_k: np.ndarray, V_k: np.ndarray,
        ...
        pred_ratings = U_k[user_idx, :] @ V_k

        movie_scores = {
            movie_name: pred_ratings[movie_idx]
            for movie_name in movies_pool
            if (movie_id := self.get_movie_id_by_name(movie_name)) in movies_index
            if (movie_idx := movies_index[movie_id]) not in np.where(~np.isnan(R[user_idx, :]))[0]
        }

        return np.array(sorted(movie_scores, key=movie_scores.get, reverse=True)[:top_n])
